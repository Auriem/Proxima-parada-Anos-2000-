...
exports.handler = async (event) => {
  try {
    ...
    try {
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Dados inválidos recebidos. Erro de parse: " + parseError.message }),
      };
    }

    // *** NOVO LOG AQUI ***
    console.log("send-ticket-manually: Verificando buyerInfo:", buyerInfo);
    if (!buyerInfo || !buyerInfo.name || !buyerInfo.email) {
      console.error("send-ticket-manually: Dados do comprador incompletos:", buyerInfo);
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Dados do comprador incompletos. Nome ou email ausente." }),
      };
    }
    // *** NOVO LOG AQUI ***
    console.log("send-ticket-manually: Verificando ticketInfo:", ticketInfo);
